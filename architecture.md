...
Use `start.ps1` for native Windows startup or the `compose.yaml` workflow documented in README.md. FastAPI serves the orange React interface and talks to the Pi agent, PostgreSQL and Ollama. The native launcher chooses an available loopback port and prints its URL. The local corpus contains 303 real transcripts. Optional cloud credentials remain supported through configuration. Reviewer access and generation limits remain available; workspace UUIDs are browser-owned session boundaries, not individual user accounts. Use one API worker; scaling requires shared job and limit infrastructure.
