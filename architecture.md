...
See docs/render-deployment.md and render.yaml. Production adds HTTPS Basic reviewer access, a private agent token, one active generation, and a global in-process hourly attempt limit. Health liveness is public; other routes require reviewer credentials. The workspace UUID remains a browser-owned session boundary, not an individual user account. Use one worker/replica; a scaled deployment requires shared job/limit infrastructure.
